...
async function default_1(parsedOptions) {
    const { ctx, meta, argc, options, config } = parsedOptions;
    if (argc)
        return meta.$send('存在多余的参数，请检查指令语法或将含有空格或换行的问答置于一对引号内。');
    if (!/^\d+(,\d+)*$/.exec(options.update))
        return meta.$send('参数 -u, --update 错误，请检查指令语法。');
    const ids = utils_1.splitIds(options.update);
    ...
    if (options.delete) {
        const predicate = meta.$user.authority > 2 ? () => false :
            meta.$user.authority > 2 ? d => !!(d.flag & database_1.DialogueFlag.frozen) :
                d => !!(d.flag & database_1.DialogueFlag.frozen) || d.writer !== meta.userId;
        ...
        if (removable.length)
            output.push(`已删除编号为 ${removable.join(', ')} 的问题。`);
        if (unremovable.length)
            output.push(`编号为 ${unremovable.join(', ')} 的问题因权限过低无法删除。`);
    ...
    if (hasUpdates) {
        ...
        for (const dialogue of dialogues) {
            ...
            function updateValue(key, type, value) {
            ...
            if (parsedOptions.envMode) {
                const oldGroups = utils_1.splitIds(dialogue.groups.replace(/^\*/, ''));
        ...
        if (skipSet.size)
            output.push(`问答 ${Array.from(skipSet).sort((a, b) => a > b ? 1 : -1).join(', ')} 修改时发生错误或权限不足。`);
        if (updateSet.size) {
    ...
    for (const dialogue of dialogues) {
        const groups = utils_1.splitIds(dialogue.groups.replace(/^\*/, ''));
        const output = [
        ...
        if (dialogue.probability < 1)
            output.push(`触发概率：${dialogue.probability}`);
        if (dialogue.flag & database_1.DialogueFlag.frozen)
            output.push('此问题已锁定');
