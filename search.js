...
async function default_1(parsedOptions) {
    ...
    if (!options.question && !options.answer) {
        if (!dialogues.length)
            return meta.$send('没有搜索到任何回答，尝试切换到其他环境。');
    ...
    if (!options.keyword) {
        if (!options.question) {
            if (!dialogues.length)
                return meta.$send(`没有搜索到回答“${answer}”，请尝试使用关键词匹配。`);
            ...
            if (!dialogues.length)
                return meta.$send(`没有搜索到问题“${question}”，请尝试使用关键词匹配。`);
        ...
        if (!options.question) {
            if (!dialogues.length)
                return meta.$send(`没有搜索到含有关键词“${answer}”的的回答。`);
            output.unshift(`回答关键词“${answer}”的搜索结果如下：`);
        }
        else if (!options.answer) {
            if (!dialogues.length)
                return meta.$send(`没有搜索到含有关键词“${question}”的的问题。`);
            output.unshift(`问题关键词“${question}”的搜索结果如下：`);
        }
        else {
            if (!dialogues.length)
                return meta.$send(`没有搜索到含有关键词“${question}”“${answer}”的的问答。`);
